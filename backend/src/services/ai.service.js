...
function normalizeFeedback(candidate, fallback) {
  return {
    ...
    estimatedScore: {
      total: Number(candidate?.estimatedScore?.total ?? fallback.estimatedScore.total),
      readingWriting: Number(candidate?.estimatedScore?.readingWriting ?? fallback.estimatedScore.readingWriting),
      math: Number(candidate?.estimatedScore?.math ?? fallback.estimatedScore.math),
      listening: Number(candidate?.estimatedScore?.listening ?? fallback.estimatedScore.listening)
    },
    strengths: Array.isArray(candidate?.strengths) ? candidate.strengths : fallback.strengths,
    weaknesses: Array.isArray(candidate?.weaknesses) ? candidate.weaknesses : fallback.weaknesses,
    ...
    studyRoadmap: Array.isArray(candidate?.studyRoadmap) ? candidate.studyRoadmap : fallback.studyRoadmap,
...
export async function generateFeedbackForAttempt(attemptId, user) {
  ...
  if (attempt.status === 'IN_PROGRESS') {
    throw new ApiError(400, 'Submit the attempt before generating AI feedback.');
  }
  ...
  try {
      ...
      messages: [
        ...
        },
        {
          ...
          content: JSON.stringify({
            instruction: 'Generate coaching feedback based only on this SAT-style attempt summary.',
    ...
    return saveFeedback(attemptId, attempt.userId, normalizeFeedback(parsed, fallback));
  } catch (error) {
    return saveFeedback(attemptId, attempt.userId, fallback);
  }
}
